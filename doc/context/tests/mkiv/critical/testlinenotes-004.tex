...
\startitemize
\item \type{\textstyle}. How do I style the footnote numerals separate from the text style? \type{style=} covers both. Eg., footnote numerals bold, text in smallcaps.  {\bf high}
\item \type{\setuplinenote [linenote] [numbercommand=\low]} compiles, but there is no effect.  {\bf low}
\item If there are related styling options, let's add them here.
\stopitemize
...
\startbuffer[test]
\setuplinenumbering[style=\bf]
\setuplinenote [linenote] [style=\it,rule=off,frame=on,framecolor=darkred]
\startlinenumbering
...
bodyfont=14pt, % bodyfont=big,
rule=off,frame=on,framecolor=darkred]
\startlinenumbering
test test test \dorecurse{40}{test }.
\linenote {A simple linenote does not have a number range}
\startlinenote [one] {A linenote environment has a range that covers the
...
\startbuffer[test]
\setuplinenumbering[style=\bf]
\setuplinenote [linenote] [numbercommand=\low,rule=off,frame=on,framecolor=darkred]
\startlinenumbering
test test test \dorecurse{40}{test }.
\linenote {A simple linenote does not have a number range}
\startlinenote [one] {A linenote environment has a range that covers the
